...
function buildFullPrompt({ workspace, message, mode, conversationHistory, attachments }) {
  ...
  if (mode === 'build') {
    parts.push(
      ...
      'Data pattern: import useAppStore from "../../stores/useAppStore.js"; const workspaceId = useAppStore((s) => s.activeWorkspaceId); then fetch data with useEffect and useState.',
      'Use native fetch (not the api utility) since it is simpler: fetch(`/api/data/summary?workspace=${workspaceId}`).then(r => r.json()).then(setData).',
      'Always guard against missing workspaceId: if (!workspaceId) return early in useEffect.',
      ...
      'Fetch data from these exact API endpoints — always append ?workspace=${workspaceId} where workspaceId comes from useAppStore((s) => s.activeWorkspaceId):',
      '  GET /api/data/summary?workspace=ID  → { totalMembers, segmentCounts:{ghost,one_and_done,approaching_threshold,in_the_zone,power_user,new_member}, avgHealthScore, ghostPercentage, firstUseRate }',
      '  GET /api/data/segments?workspace=ID  → array of { segment, count, avgHealth, pct }',
      '  GET /api/data/members?workspace=ID&limit=N&offset=N&segment=NAME  → { members:[...], total }',
      '  GET /api/data/metrics?workspace=ID  → { renewalRate, avgVisits, topMarkets:[{market,count}], channelBreakdown:[{channel,count}] }',
      '  GET /api/data/migration?workspace=ID  → segment migration data',
      '  Do NOT call any other endpoints. Do NOT use fetch without the workspace query param.',
